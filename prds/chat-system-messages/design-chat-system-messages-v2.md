...
| **Этап** | 2 из 5 |
| **Версия** | 2.0 (Draft) |
| **Предыдущая версия** | [v1.0](design-chat-system-messages.md) |
| **Дата** | 2026-09-13 |
| **PRD** | [`prd-video-chat-room.md`](../../prd-video-chat-room.md) v1.0 |
| **Зависит от** | [1 — room-skeleton](../room-skeleton/design-room-skeleton-v2.md): общая архитектура, контракт, `RoomRegistry`, `RoomSession`, reducer |
...
|---|---|---|
| §1, §4.3.1, §8, §11, §13, §14 | Системные сообщения: «**Имя** присоединился» / «**Имя** отключился», единый шаблон для себя и для других | Решение по TBD-1 v1 |
| §1, §4.3, §4.3.3, §8, §10, §11, §13, §14 | Кликабельные http(s)-ссылки (linkify) | Решение по TBD-2 v1 |
| Шапка | Ссылка на этап 1 ведёт на v2 | — |
...
- системные сообщения «**Имя** присоединился» / «**Имя** отключился», одинаковые для осознанного выхода и обрыва;
- кликабельные `http(s)://`-ссылки в тексте сообщений;
...
**Не входит:** файлы, редактирование и удаление, личные сообщения, реакции, звуки (PRD, Non-Goals).

> **Linkify входит в скоуп по решению v2**, хотя PRD относит расширенный чат к Non-Goals. Это только отображение: модель данных, контракт `chat:*` и серверная валидация не меняются.
...
Состояние **после этапа 1** (файлы, спроектированные в [design-room-skeleton.md](../room-skeleton/design-room-skeleton-v2.md)). Кода в репозитории на момент написания нет.
...
| `features/chat/UserMessageItem.tsx` | Имя автора, время `formatTime(ts)`, текст через `<MessageText text={message.text} />` (§4.3.3). Для `authorId === selfId` — модификатор `--own` |
| `features/chat/SystemMessageItem.tsx` | Текст из `formatSystemMessage(message)`, приглушённый стиль, время |
...
| `features/chat/linkify.ts` | Чистая функция: текст → сегменты «текст / ссылка» (§4.3.3) |
| `features/chat/MessageText.tsx` | Рендер сегментов: текстовые узлы и безопасные `<a>` (§4.3.3) |
...
##### 4.3.1 Тексты системных сообщений (v2)

| Событие | Текст | Когда |
|---|---|---|
| `participant-joined` | «**Алекс** присоединился» | Вход любого участника, включая себя (последний пункт своей истории — сообщение с собственным именем) |
| `participant-left` | «**Алекс** отключился» | И «Выйти», и закрытие вкладки, и обрыв: сервер их не различает (FR-31). Слово «отключился» не называет причину, так что требование не писать «соединение потеряно» соблюдено |

- Единый шаблон для всех, отдельной формы «Вы присоединились» нет.
- Имя выделено `<strong>` и подставлено текстовым узлом JSX: `<><strong>{name}</strong> присоединился</>`. HTML-шаблонов нет.
- Грамматический род не согласуется: формулировки зафиксированы решением v2.
- `formatSystemMessage(message)` возвращает `{ name: string; action: 'присоединился' | 'отключился' }`, разметку строит `SystemMessageItem`.
...
##### 4.3.3 Кликабельные ссылки (linkify, v2)

`features/chat/linkify.ts` — чистая функция без DOM:

```ts
export type TextSegment =
  | { kind: 'text'; value: string }
  | { kind: 'link'; value: string; href: string };   // value — как в исходном тексте, href — URL.href

/** Находит http(s)-ссылки. Другие схемы никогда не возвращает. */
export function linkify(text: string): TextSegment[];
```

Алгоритм:
1. Кандидаты ищутся регуляркой `/\bhttps?:\/\/[^\s<>"'\`]+/giu`.
2. Хвостовая пунктуация (`. , ; : ! ? » " '`) и закрывающие `) ] }` без парной открывающей внутри кандидата отрезаются и остаются текстом. `(см. https://ru.wikipedia.org/wiki/Mesh_(топология))` даёт ссылку со сбалансированной скобкой, а внешняя `)` остаётся текстом.
3. `new URL(candidate)` в `try/catch`. Ссылка создаётся, только если `url.protocol` равен `http:` или `https:`, иначе фрагмент остаётся текстом.
4. Ссылки без схемы (`www.example.com`, `example.com`) **не** распознаются: так меньше ложных срабатываний.

`features/chat/MessageText.tsx` (эскиз):
    ...
    <>
      {segments.map((s, i) =>
        s.kind === 'text'
          ...
          : <a key={i} href={s.href} title={s.href} target="_blank" rel="noopener noreferrer nofollow">{s.value}</a>,
      )}
    </>
  );
});
...
| Правило | Зачем |
|---|---|
| Только `http:` / `https:`, проверка после разбора `URL` | Исключает `javascript:`, `data:`, `vbscript:`, `file:` |
| `href` берётся из `URL.href` (нормализованный), текст ссылки — исходный | Нормализация кодирует пробелы и спецсимволы; атрибут экранирует React |
| `target="_blank"` + `rel="noopener noreferrer nofollow"` | Нет tabnabbing (`window.opener`), URL комнаты не утекает в `Referer` |
| `title={href}` | `URL.href` переводит IDN-домен в punycode, и при наведении видно `xn--…` — частичная защита от гомографов |
| Разбор на клиенте при рендере, сервер хранит сырой текст | Контракт `chat:message` и валидация не меняются, CSP не затрагивается |
...
| `javascript:alert(1)`, `data:text/html,…` в тексте | Остаётся текстом: linkify распознаёт только `http(s)://` и перепроверяет `URL.protocol` |
| Ссылка в конце предложения (`https://example.com/a.`) или в скобках | Точка и непарная скобка в ссылку не входят |
| `www.example.com` без схемы | Остаётся текстом (§4.3.3) |
...
| Последний участник вышел, кто-то вошёл тем же URL | Новая `Room` с пустой историей (FR-9). В ленте только «**Имя** присоединился» с собственным именем |
...
| Угроза | Мера |
|---|---|
| Stored XSS через текст или имя | React-текстовые узлы, `react/no-danger: error`; linkify строит `<a>` через JSX и только для `http(s)`. Сервер хранит сырой нормализованный текст и **не** экранирует его в HTML (иначе будет двойное экранирование) |
...
| Опасные ссылки (`javascript:`, tabnabbing, гомографы) | Whitelist схем после `new URL`, `rel="noopener noreferrer nofollow"`, `title` с punycode (§4.3.3) |
| Подделка автора | и `authorName` берутся из серверного `Participant` сокета, а не из payload |
...
| `client/formatSystemMessage` | Вход → `{ name, action: 'присоединился' }`, выход → `'отключился'` |
| `client/linkify` | `https://a.b/c` → ссылка; `http://x.y/(z)` сохраняет парную скобку; хвостовые `.,!?»)` отрезаются; `javascript:alert(1)`, `data:…`, `ftp://…`, `www.a.b` → текст; невалидный URL → текст; несколько ссылок подряд; IDN-домен → `href` в punycode |
...
| `SystemMessageItem` | «**Алекс** присоединился» и «**Алекс** отключился»: имя в `<strong>`, `<b>x</b>` в имени рендерится текстом |
| `MessageText` | Ссылка — `<a>` с `target="_blank"` и `rel="noopener noreferrer nofollow"`; в DOM нет `a[href^="javascript"]` для текста `javascript:alert(1)` |
...
4. Системные сообщения: C входит → у A «C присоединился»; C закрывает вкладку → у A «C отключился».
...
7. Ссылка: A отправляет «см. https://example.com/doc.» → у B ссылка с `href="https://example.com/doc"` без точки; клик открывает новую вкладку (`context.waitForEvent('page')`, запрос перехвачен `context.route`), в ней `window.opener === null`.
...
| Linkify ошибается: ложные срабатывания, обрезанные ссылки со скобками или нелатинскими символами | Консервативные правила (§4.3.3), таблица unit-кейсов; при сомнении фрагмент остаётся текстом |
...
**Решено в v2:**
- Формулировки системных сообщений: «**Имя** присоединился», «**Имя** отключился» (§4.3.1).
- Linkify нужен: только `http(s)://`, безопасные атрибуты ссылок (§4.3.3).

**Открыто:**
1. **TBD:** Выделять ли имя в системном сообщении жирным (`<strong>`)? Так прочитана разметка `*Имя_Пользователя*` из решения. Подтвердить.
2. **TBD:** Распознавать ли ссылки без схемы (`www.example.com`)? Сейчас — нет.
3. **TBD:** «Умная» автопрокрутка: не прокручивать, если пользователь отлистал вверх, и показать «↓ Новые сообщения»? PRD требует безусловной прокрутки. Предлагаю оставить как Should на будущее.
4. **TBD:** Значения по умолчанию: `MESSAGE_MAX_LENGTH = 1000`, `CHAT_HISTORY_LIMIT = 200`, rate limit `5 burst / 1 в секунду`. Подтвердить.

**Решено по умолчанию:** своё сообщение появляется после broadcast от сервера, без оптимистичного рендера.
