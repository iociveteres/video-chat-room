...
- сквозная приёмка: E2E с 4 участниками + 5-й «Комната заполнена», ручной кросс-браузерный прогон на реальных устройствах.
...
| CPU клиента | < 70% на эталонном ноутбуке (**TBD:** модель) | Диспетчер задач / Activity Monitor, 10-минутный звонок |
...
- [ ] 4 разных устройства в LAN (Chrome, Firefox, Edge, минимум Windows + macOS), 10-минутный звонок.
- [ ] Задержка glass-to-glass ≤ 500 мс (метод §9.2), результаты занесены в отчёт.
- [ ] CPU на эталонном ноутбуке < 70%, `qualityLimitationReason` в основном `none`.
- [ ] Выключение камеры → лампочка гаснет у всех браузеров.
- [ ] Отключение Wi-Fi у одного → остальные продолжают звонок, через ≤ 15 с системное сообщение о выходе.
- [ ] 5-й участник → «Комната заполнена», лампочка у него гаснет.
- [ ] Отказ в доступе к камере у одного → у остальных силуэт и имя; он слышит и видит других.
- [ ] Все сценарии PRD US-1…US-13 пройдены по Gherkin-критериям.

### 11.6 Покрытие

Итоговые цели продукта: `shared` ≥ 95%, `server` ≥ 90%, `client/state` + `client/call` + `client/media` ≥ 90%, `client` в целом ≥ 75%.
...
- **Релиз v1.0** после этапа 5:
  1. Зелёный CI (lint, typecheck, unit, integration, property-based, E2E).
  2. Пройден ручной release checklist (§11.5).
  3. README: требования (Node ≥ 22), `npm ci && npm run dev`, запуск по LAN с HTTPS, известные ограничения (без TURN, эхо при нескольких устройствах в одном помещении, раскрытие IP в P2P).
  4. Git-тег `v1.0.0`.
